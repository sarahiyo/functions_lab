...
var evenNumbers = myNumbers.filter(function(number){
    return number % 2 === 0;
})
...
function getTotalBill(myBill){
  var total = 0
  for (var item of myBill){
    var itemPrice = item.price
    total += itemPrice
  }
  return total
}
console.log("total price:", getTotalBill(myBill))
...
function getIndividualBill(myBill){
  for (var currentBillObj of myBill){
      var customerNameToFind = "Sally"
      var customerMeal = currentBillObj.meal
      if (customerNameToFind === "Sally")
      console.log("Sally had:", customerMeal(myBill))
  }
}
